      ...
      if (tab === 'agents') {
        ...
        setItems(
          (res.items || []).map((it: any) => ({
            ...
            name: it.title || it.name || it.identifier,
            description: it.description || '',
            icon: it.avatar || it.meta?.avatar,
            category: it.category,
            author: it.author,
            installCount: it.installCount ?? it.cloneCount,
            ratingAvg: it.ratingAvg ?? it.rating,
        ...
        const res = await discoverService.getPluginList({
          page,
          pageSize: PAGE_SIZE,
          category: category === 'all' ? undefined : category,
          q: keyword || undefined,
        ...
        setItems(
          (res.items || []).map((it: any) => ({
            identifier: it.identifier,
            name: it.meta?.title || it.name || it.identifier,
            description: it.meta?.description || it.description || '',
            icon: it.meta?.avatar || it.icon,
            category: it.category,
            author: it.author,
            installCount: it.installCount,
            isFeatured: it.isFeatured,
            isOfficial: it.isOfficial,
            tags: it.meta?.tags || it.tags || [],
            raw: it,
          })),
        );
        setCategories([]);
        ...
        const res = await discoverService.getModelList({
          page,
          pageSize: PAGE_SIZE,
          q: keyword || undefined,
        });
        setItems(
          (res.items || []).map((it: any) => ({
            identifier: it.identifier,
            name: it.displayName || it.identifier,
        ...
        const res = await discoverService.getProviderList({
          page,
          pageSize: PAGE_SIZE,
          q: keyword || undefined,
        });
        setItems(
          (res.items || []).map((it: any) => ({
            ...
            description: it.description || it.modelCount + ' 个模型',
            icon: it.avatar,
            author: it.identifier,
            installCount: it.modelCount,
            raw: it,
          })),
        );
        setCategories([]);
    ...
    async (item: MarketItemCard) => {
      ...
      try {
        if (tab === 'agents') {
          ...
          msg.success(res?.message || '已克隆到工作区，可在对话中使用');
        } else if (tab === 'skills') {
          await agentSkillService.importFromMarket(item.identifier);
          msg.success('技能安装成功，可在 Agent 技能库中管理');
        } else if (tab === 'mcp') {
          ...
          if (typeof store.installMCPPlugin === 'function') {
            ...
            if (ok === false) msg.info('安装向导已打开，请按提示完成配置');
            else msg.success('MCP 已安装');
          } else {
            msg.info('请前往设置 → MCP 中添加该服务器');
          }
        } else {
          msg.info('该类型支持浏览与预览，安装请前往对应设置页');
        }
      } catch (e: any) {
        msg.error(e?.message || '安装失败');
      } finally {
        setInstallingId('');
      }
    },
        ...
        <Flex align="center" gap={10}>
          ...
          <Flex vertical flex={1} style={{ minWidth: 0 }}>
            ...
            <Space size={4} wrap>
              ...
              {item.category && (
                <Tag style={{ fontSize: 10, marginInlineEnd: 0 }}>{item.category}</Tag>
              )}
            </Space>
          </Flex>
        </Flex>
    ...
    <Flex vertical gap={12} style={{ height: '100%', padding: 16 }}>
      ...
      <Flex justify="space-between" align="center" wrap gap={8}>
        <Space size={4} wrap>
          <Tag
            key="all"
            color={category === 'all' ? 'blue' : undefined}
      ...
      <Drawer
        ...
        title={
          <Space>
            <span style={{ fontSize: 22 }}>{detailItem?.icon || '🤖'}</span>
        ...
        {detailLoading ? (
          ...
          <Flex vertical gap={12}>
            <Descriptions column={2} size="small" bordered>
              ...
              {detailItem?.author != null && (
                <Descriptions.Item label="作者">{detailItem.author}</Descriptions.Item>
              )}
              {detailItem?.installCount != null && (
                <Descriptions.Item label="安装">{detailItem.installCount}</Descriptions.Item>
              )}
            </Descriptions>
