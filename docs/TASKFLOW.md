...
| S8-11 | **触发 proc 后 SSH 上服务器查打包**：proc 看 GIT 自动部署日志 /git log 最新 hash/front 产物时间戳 /bun install+build 结果 /nginx reload；后端看 docker ps/logs/migration/7001 探活；失败即修复重推直至成功                                                   | 部署架构见 C-END-ANALYSIS §5                                                                                         | P5-3/4        | 🔄 进行中（前端产物已部署；nginx conf 已手动应用；**后端部署失败待排查**） |
| S8-12 | 数据库校验（8 个 migration + 25 张表，只读）                                                                                                                                                                                 | SSH 115.190.150.8                                                                                               | P5-1/2        | 🔄 进行中（migrations 最新 1790000007000，新迁移随后端部署执行） |
...
| S8-16 | **遗留**：聊天页消息气泡渲染（conversation store messagesInit 同步 + 刷新历史加载）；后端链路已全通 | TESTCASES-v2 遗留表 | P1-CHAT-10/11 | PENDING（需真实键盘交互调试） |
